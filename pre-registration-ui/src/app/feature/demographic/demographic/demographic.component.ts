...
export class DemographicComponent
  ...
  */
  ...
  async loadFieldDefinitions() {
    const response = await fetch('assets/data/fieldDefinitions.json');
    ...
    if (field && field.hasOwnProperty("dependentFields")) {
      ...
      for (const dependentField of dependentFields) {
        ...
        if (subField) {
          ...
          else if (subField.hasOwnProperty("requiredCondition")) {
  ...
  processShowHideFields = async (formIdentityData: any, subField?: any) => {
    if (subField) {
      console.log("called with subField");
      let facts = {};
      ...
      if (subField.parentField) {
        console.log(subField.parentField);
  ...
  processConditionalRequiredValidations(identityFormData, uiField) {
    ...
    if (uiField && uiField.requiredCondition && uiField.requiredCondition != "") {
      ...
      if (uiField.parentField) {
        const parentFieldValue = identityFormData.identity[uiField.parentField];
        facts = { [uiField.parentField]: parentFieldValue };
        console.log(facts)
      }
